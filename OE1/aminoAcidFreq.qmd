...
freRel_list <- lapply(fasta_list, function(fasta) {
  ...
  data <- lapply(fasta, function(seq) {
    ...
    as.data.frame(freq) %>% mutate(AminoAcid = rownames(.))  # Formatear a dataframe
  })
...
all_frequencies <- bind_rows(freRel_list, .id = "File")  # Identificar el archivo de origen
...
ggplot(all_frequencies, aes(x = AminoAcid, y = Freq, fill = AminoAcid)) +
  geom_boxplot() +
  theme_minimal() +
  labs(title = "Frecuencia de Aminoácidos por Archivo",
       x = "Aminoácido",
       y = "Frecuencia Relativa (%)") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  facet_wrap(~ File, scales = "free_y")  # Un gráfico por archivo
