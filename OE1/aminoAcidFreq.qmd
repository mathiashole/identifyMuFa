...
classification_aminoacids <- function(data, genome_col = "File", id_col = "Sequence_ID", type) {
    result_combained_data <- data.frame(
      ...
      "id" = data[[ id_col]],
      "Aliphatic (G,A,V,L,I)" = data[,"G"] + data[,"A"] + data[,"V"] + data[,"L"] + data[,"I"],
      "Aromatic F,W,Y" = data[,"F"] + data[,"W"] + data[,"Y"],
      "Sulphur C,M" = data[,"C"] + data[,"M"],
      "Basic K,R,H" = data[,"K"] + data[,"R"] + data[,"H"],
      "Acidic B,D,E,N,Q,Z" = data[,"D"] + data[,"E"] + data[,"N"] + data[,"Q"],
      "Aliphatic hydroxyl S,T" = data[,"S"] + data[,"T"],
      "tRNA synthetase class I Z,E,Q,R,C,M,V,I,L,Y,W" = data[,"E"] + data[,"Q"] + data[,"R"] + data[,"C"] + data[,"M"] + data[,"V"] + data[,"I"] + data[,"L"] + data[,"Y"] + data[,"W"],
      "tRNA synthetase class II B,G,A,P,S,T,H,D,N,K,F" = data[,"G"] + data[,"A"] + data[,"P"] + data[,"S"] + data[,"T"] + data[,"H"] + data[,"D"] + data[,"N"] + data[,"K"] + data[,"F"]
    ) # fix data[,"B"], data[,"Z"]
}
