...
```{r}
#| label: extract feature of key_name
#| echo: true
#| warning: false
...
```{r}
#| label: filter all the proteins by extracting the target sequence
#| echo: true
#| warning: false

filtered_all_code_protein <- anti_join(all_code_prot_data, target_protein, by = "ID")

filter_all_code_prot_summary <- filtered_all_code_protein %>%
  group_by(file) %>%
  summarise(Total_Sequences = n(),
            min_length = min(Length),
            mean_length = round(mean(Length), 0),
