...
plot_violin_length <- function(data, type, plot_title, x_label) {
  set.seed(12345)
  p <- ggplot(data, aes(Length, file, color = file)) + # reorder(file, -Length, FUN=median)
...
Por otro lado, creamos otra funcion para crear graficos violin sin escala logaritmica para representar secuencias de menor tamano como son los genes. La función la llamamos como`plot_violin_length` crea un gráfico de violín de la distribución de los largos de las diferentes secuencias de ``r key_name``.
