...
Se llama a la funcion ``calculate_fasta_feature`` para calcular por cada secuencia codificante ``r rest_code_prot`` el contenido de `GC` y la `longitud (pb)`. Finalmente, creamos una `tabla resumen`, que agrupa las secuencias por cada genoma ``r genome_name``, mostrando el  ``r rest_code_prot``, así como la `longitud` mínima, media y máxima, así como el contenido de `GC` mínimo, medio y máximo.
