...
En esta sección, se generarán gráficos que representan el `contenido de GC` y la `longitud` de cada genoma. Además, se implementarán funciones específicas para cada gráfico, lo que permitirá reutilizar el código de manera eficiente en otras partes del análisis.
...
Por otro lado, Lla función de R `plot_violin_length` crea un gráfico de violín que ilustra la distribución de las longitudes genómicas de los diferentes genomas ``r genome_name``.
...
plot_violin_length <- function(data, plot_title, x_label) {
  set.seed(12345)
  ggplot(data, aes(Length, reorder(file, -Length, FUN=median), color = file)) +
...
# Crear el violin plot para GC_Content
ggplot(all_results, aes(x = file, y = GC_Content, fill = file)) +
  geom_violin(alpha = 0.7, width = 0.5) + 
  geom_boxplot(width = 0.1, fill = "white", alpha = 0.3) +
  # geom_jitter(width = 0.1, alpha = 0.1, color = "black") +
  scale_fill_brewer(palette = "Dark2") +  # Aplicar la paleta Dark2 de RColorBrewer
  ylab("Number of GC content") +
  labs(
    title = "Genome GC",
    caption = "Data source: Facultad de Ciencias, Udelar"
  ) +
