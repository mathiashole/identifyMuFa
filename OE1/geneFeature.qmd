...
En el caso particular de procesar las secuencias fatas creamos una funcion que genera un dataframe vacio donde se iran guardando los elementos obtenidos a partir de una secuecnia dada. Luego, borramos el path de cada archivo para obtener el nombre de cada archivo, se obtiene el ide de cada unas de las secuencias dentro del archiv fasta, se calcula el largo de la secuencia con una funcion de la biblioteca SEQINR y se calcula el porcentje de GC con otra funcion de la misma biblioteca.. Finalmente, agregamos una fila como header y devolvmos el resultado de la funcion.
