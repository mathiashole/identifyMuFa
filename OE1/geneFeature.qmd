...
plot_violin_length <- function(data, plot_title, x_label) {
  set.seed(12345)
  ggplot(data, aes(Length, file, color = file)) + # reorder(file, -Length, FUN=median)
...
Por otro lado, llamamos a la función de R `plot_violin_length` crea un gráfico de violín de la distribución de los largos de las diferentes secuencias de ``r key_name``.
