...
calculate_fasta_feature <- function(fasta_files){
...
# calculate_fasta_feature <- function(fasta_files, type_fasta) {
#   all_results <- data.frame() # Inicializar el data frame para almacenar los resultados
#   if (type_fasta == "genome") {
#     all_results <- data.frame(file = character(), Length = numeric(), GC_Content = numeric(), stringsAsFactors = FALSE)
#   } else {
#     all_results <- data.frame(file = character(), ID = character(), Length = numeric(), GC_Content = numeric(), stringsAsFactors = FALSE)
#   }
...
#   for (fasta_file in fasta_files) {
#     fasta_content <- read.fasta(fasta_file)
#     extracted_names <- sub(".*_([^_]+)\\.fasta$", "\\1", basename(fasta_file))

#     if (type_fasta == "genome") {
#       total_length <- sum(sapply(fasta_content, getLength))
#       total_gc <- sum(sapply(fasta_content, function(seq) GC(seq) * getLength(seq)))
#       genome_gc_content <- round(total_gc / total_length, 3)

#       all_results <- all_results %>%
#         add_row(file = extracted_names, Length = total_length, GC_Content = genome_gc_content)
#     } else {
#       for (sequence in fasta_content) {
#         id <- attr(sequence, "name")
#         length <- round(getLength(sequence), 0)
...
epa <- calculate_fasta_feature(data_init$V1, "genome")
