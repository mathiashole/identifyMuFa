...
create_pca_highlight_plot(combained_dinucleotide, dinucleotide_table$ID, perform_result_dinucleotide_pca$pca_result)
...
create_pca_highlight_plot(combained_trinucleotide, trinucleotide_table$ID, perform_result_trinucleotide_pca$pca_result)
...
for (genome in genomes) {
  ...
  pca_plot <- create_pca_highlight_plot(genome_data, dinucleotide_table$ID, pca_result$pca_result, col_palette = palette_to_genome)
...
for (genome in genomes) {
  ...
  pca_plot <- create_pca_highlight_plot(genome_data, trinucleotide_table$ID, pca_result$pca_result, col_palette = palette_to_genome)
...
perform_tsne <- function(data, file_col = "File", id_col = "ID", perplexity = 30, max_iter = 1000) {
  # Exclude 'File' and 'ID' columns from numerical analysis
  ...
  tsne_data <- tsne_data[!duplicated(tsne_data), ] # NEED CHECK WITH BOSS

  # Perform t-SNE
  tsne_result <- Rtsne(tsne_data, perplexity = perplexity, max_iter = max_iter)
  ...
  # Plot the t-SNE using ggplot2
  tsne_plot <- ggplot(tsne_scores, aes(x = tSNE1, y = tSNE2, color = .data[[file_col]])) +
...
# Call perform_tsne
perform_result_dinucleotide_tsne <- perform_tsne(combained_dinucleotide, file_col = "Genome", id_col = "id")
