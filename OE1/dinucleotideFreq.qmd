...
calculate_nucleotide_frequencies <- function(directory, type = "dinucleotide") {
  # fasta_files <- list.files(path = directory, pattern = "^out_.*\\.fasta$", full.names = TRUE)
  ...
  for (fasta_file in fasta_files) {
    ...
    for (i in seq_along(sequences)) {
      ...
      if (type == "trinucleotide") {
        freqs <- count_trinucleotides(seq)
      } else {
        freqs <- count_dinucleotides(seq)
      }
      
      row <- c(File = extracted_names, ID = id, freqs)
...
dinucleotide_table <- calculate_nucleotide_frequencies(directory, type = "dinucleotide")
