...
# calculate_nucleotide_frequencies <- function(fasta_files, type_frequency = "dinucleotide") {
#   # fasta_files <- list.files(path = directory, pattern = "^out_.*\\.fasta$", full.names = TRUE)
...
calculate_dinucleotide_frequencies <- function(fasta_files) {
  results <- data.frame()  # Inicializa el data frame vacío para los resultados
  ...
  for (fasta_file in fasta_files) {
    ...
    for (i in seq_along(sequences)) {
      seq <- getSequence(sequences[[i]])
      freqs <- count_dinucleotides(seq)  # Calcula las frecuencias de dinucleótidos
...
dinucleotide_table <- calculate_dinucleotide_frequencies(fasta_files)

# Run the function and get the dinucleotide frequency table
dinucleotide_table <- calculate_nucleotide_frequencies(fasta_files, type_frequency = "dinucleotide")

dinucleotide_table_all_prot_code <- calculate_nucleotide_frequencies(all_prot_code, type_frequency = "dinucleotide", "gene")
