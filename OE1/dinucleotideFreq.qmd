...
rest_code_prot <- list.files(path = "/home/usuario/BASH/chack_gff/prueba_frecuencia_nucleotidos", pattern = "^out_.*\\.fasta$", full.names = TRUE)
...
create_pca_highlight_plot <- function(data, highlight_ids, pca_result, genome_col = "Genome") {
...
for (genome in genomes) {
  ...
  pca_plot <- create_pca_highlight_plot(genome_data, dinucleotide_table$ID, pca_result$pca_result)
...
for (genome in genomes) {
  ...
  pca_plot <- create_pca_highlight_plot(genome_data, trinucleotide_table$ID, pca_result$pca_result)
