...
Análisis de componentes principales (PCA) sobre las frecuencias de dinucleótidos combinados en las secuencias codificantes de los genomas ``r genome_name``. El PCA se utiliza para reducir la dimensionalidad, en este caso de las frecuencias de dinucleótidos en secuencias codificantes de los `genomas`, lo que permite explorar las relaciones entre secuencias y sus patrones de composición a nivel de dinucleótidos. Aquí se utiliza la función `autoplot()` para visualizar los resultados del `PCA`. Las secuencias se colorean según los `genomas`, mostrando los vectores de carga (loadings) para ver qué dinucleótidos influyen más en cada componente principal.
...
# Crear la nueva columna 'highlight' que combina 'Highlighted' con la columna 'File'
...
# Asignar los valores de transparencia según la columna 'highlight'
dinucleotide_all_protein_coding$alpha <- ifelse(
  grepl("Highlighted", dinucleotide_all_protein_coding$highlight), 0.8, 0.1
)

# Asignar las primeras dos componentes principales del PCA
dinucleotide_all_protein_coding$PC1 <- pca_result$x[, 1]
dinucleotide_all_protein_coding$PC2 <- pca_result$x[, 2]

# Obtener los valores únicos de la columna 'highlight'
unique_highlight_values <- unique(dinucleotide_all_protein_coding$highlight)

# Crear una paleta de colores que asigna un color gris a 'Other' y colores distintos a cada 'Highlighted_<File>'
color_palette <- c("grey", rainbow(length(unique_highlight_values) - 1))

# Mapeo de colores: asignamos 'grey' a 'Other' y un color distinto a cada 'Highlighted_<File>'
names(color_palette) <- unique_highlight_values

# Graficar con ggplot, diferenciando cada 'Highlighted_<File>'
ggplot(dinucleotide_all_protein_coding, aes(x = PC1, y = PC2, color = highlight, alpha = alpha)) +
  geom_point(size = 3) +
  scale_color_manual(values = color_palette) +  # Aplicar los colores generados
  labs(title = "PCA Highlighting Selected IDs", x = "Principal Component 1", y = "Principal Component 2") +
  theme_minimal() +
  guides(alpha = "none")


# Graficar con autoplot para los valores de PCA
