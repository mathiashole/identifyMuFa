...
## 🧬 Visualización del Contenido de GC y Longitud de las Secuencias Codificantes

Se realizó un análisis descriptivo de las secuencias codificantes de las distintas cepas, enfocándose en su `contenido de GC (%)` y `longitud (en pares de bases)`.

Se utilizaron funciones en R reutilizables para automatizar la generación de gráficos en esta y otras etapas del análisis.

📊 Tambien, se construye una tabla resumen por genoma ``r genome_name`` de las secuencias codificantes, que incluye los valores mínimos, medios y máximos tanto de la longitud como del contenido de GC.
...
### 🧬 Análisis del genes ``r key_name``

Se analizó la distribución del `contenido de GC (%)` y la `longitud (pb)` de las secuencias correspondientes los genes `r key_name` en cada genoma `r genome_name`.

Las visualizaciones permiten observar si estas secuencias presentan comportamientos distintos respecto al total de las secuencias codificantes o al genoma completo.
...
La función ``plot_violin_gc`` muestra la distribución del `contenido de GC (%)` de las secuencias de los genes `r key_name`.
📦 Paquetes: ``ggplot2``, ``ggridges``, ``ggbeeswarm``.
...
## 📏 Gráfico de Violín de la Longitud

La función ``plot_violin_length`` visualiza la `longitud (pb)` de las secuencias de los genes `r key_name`.
📦 Paquetes: ``ggplot2``, ``ggridges``, ``ggbeeswarm``.
...
### 🔎 Gen de Interés Filtrado por Longitud

Se aplicó un filtro de calidad excluyendo secuencias del gen de interés con una longitud menor al umbral establecido. Esto permite observar sólo las secuencias con longitud representativa.

::: {.panel-tabset .nav-pills}

## 🎻 Gráfico de Violín del Contenido de GC

La función `plot_violin_gc` representa la distribución del `contenido de GC (%)` de las secuencias del genes `r key_name` filtradas por longitud.
📦 Paquetes: ``ggplot2``, ``ggridges``, ``ggbeeswarm``.
...
La función plot_violin_length muestra la distribución de longitudes (pb) de las secuencias del gen de interés filtradas.
📦 Paquetes: ggplot2, ggridges, ggbeeswarm.
