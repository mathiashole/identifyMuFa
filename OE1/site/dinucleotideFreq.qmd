...
perform_tsne_highlight <- function(data, highlight_ids, file_col = "File", id_col = "ID", perplexity = 30, max_iter = 500, col_palette = NULL) {
  # Exclude 'File' and 'ID' columns from numerical analysis
  tsne_data <- data[, !(names(data) %in% c(file_col, id_col))]
  # Convert all columns to numeric format
  num_cols <- 3:min(12, ncol(tsne_data))
  tsne_data[num_cols] <- lapply(tsne_data[num_cols], as.numeric)
  # removed duplicated row
  duplicates <- duplicated(tsne_data)
  tsne_data_clean <- tsne_data[!duplicates, ]
  tsne_data_clean <- na.omit(tsne_data_clean)
